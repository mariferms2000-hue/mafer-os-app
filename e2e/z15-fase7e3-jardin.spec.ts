...
async function seedCompletedFixtures() {
  const rows: string[] = [];
  for (let i = 0; i < 15; i++) {
 ...
 *  (Postgres exige un booleano en WHERE: `false`, no el `0` de SQLite.) */
async function cleanFixtures() {
  await run(
    `CREATE TABLE IF NOT EXISTS _z15_respaldo AS SELECT * FROM focus_plants WHERE false;
...
test("la pestaña «Mi jardín» existe en Explorar y muestra la planta actual real", async ({ page }) => {
  ...
  if (!(await activePlant())) await completeArranque(page); // base fresca: la primera sesión hace nacer la planta
...
test("estado vacío sereno cuando no hay plantas completadas (con respaldo restaurado intacto)", async ({ page }) => {
  ...
  const before = (await q<{ n: number }>("SELECT COUNT(*) n FROM _z15_respaldo"))[0].n;
  ...
  const after = (await q<{ n: number }>("SELECT COUNT(*) n FROM focus_plants WHERE completed_at IS NOT NULL"))[0].n;
